...
using Dataocean.Venus.Tests.TestDatas;
...
namespace Dataocean.Venus.Tests
{
    ...
    /// Provides methods to easily work with <see cref="VenusDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<VenusTestModule>
    {
        ...
        {
            void NormalizeDbContext(VenusDbContext context)
            {
        ...
        protected void UsingDbContext(Action<VenusDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<VenusDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<VenusDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<VenusDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<VenusDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<VenusDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<VenusDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<VenusDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<VenusDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<VenusDbContext, Task<T>> func)
        {
            T result;
